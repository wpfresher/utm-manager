...
class PluginTest extends \Codeception\TestCase\WPTestCase {
	...
	public function test_plugin_activation() {
		$plugin = wc_starter_plugin();
		$this->assertTrue( $plugin->is_plugin_active( $plugin->get_basename() ) );
		$this->assertEquals( $plugin->get_version(), get_option( 'wc_starter_plugin_version' ) );
	}
	...
	public function test_settings() {
		$settings = \WooCommerceStarterPlugin\Admin\Settings::get_instance();
