...
abstract class PremiumPlugin extends Plugin {
	...
	protected function __construct( $data ) {
		...
		if ( empty( $this->get_item_id() ) ) {
			...
			wp_die( esc_html( sprintf( __( 'The item_id is missing for %s', 'utm-manager', 'utm-manager' ), $this->data['name'] ) ) );
		}
			...
			<div class="license-notice__content" style="display:flex;align-items: center;margin:22px 0 23px;">
				...
				<div class="license-notice__text">
					<h2 style="margin:0 0 10px;padding: 0;">
						...
						<?php printf( esc_html__( 'Thanks for installing "%s"!', 'utm-manager', 'utm-manager' ), esc_html( $this->data['name'] ) ); ?>
					</h2>
					<?php
					printf(
						...
						wp_kses_post(
							...
							// translators: %1$s is the plugin name, %2$s is the license key, %3$s is the license status.
								__( 'Please activate your license to get updates and support. You can find your license key in your account on %1$surldev.com%2$s and in the email you received after purchase.', 'utm-manager', 'utm-manager' ),
					...
					<form method="post" style="display:flex;flex-direction:row;align-items:center;flex-wrap:wrap;gap: 10px;">
						...
						<input class="regular-text" type="text" name="key" placeholder="<?php echo esc_attr__( 'Enter your license key', 'utm-manager', 'utm-manager' ); ?>" required value="<?php echo esc_attr( $license ); ?>" style="margin-right:-10px; border-top-right-radius:0; border-bottom-right-radius:0; border-right:0;">
						<button type="submit" class="button button-secondary" style="border-top-left-radius:0; border-bottom-left-radius:0;line-height: 20px;"><span class="dashicons dashicons-admin-network"></span>&nbsp;<?php echo esc_html__( 'Activate', 'utm-manager', 'utm-manager' ); ?></button>
						<?php printf( '<a href="%s" target="_blank">%s</a>', esc_url( $this->data['pluginuri'] ), esc_html__( 'Buy License', 'utm-manager', 'utm-manager' ) ); ?>
						<?php printf( '<a href="https://urldev.com/support/" target="_blank">%s</a>', esc_html__( 'Contact Support', 'utm-manager', 'utm-manager' ) ); ?>
	...
	public function add_license_link( $links ) {
		...
		$action_links = array(
			'license' => sprintf(
				...
				esc_attr__( 'license', 'utm-manager', 'utm-manager' ),
				esc_html__( 'License', 'utm-manager', 'utm-manager' )
			),
		);
			...
			<td colspan="<?php echo esc_attr( $colspan ); ?>" class="plugin-update colspanchange">
				<div class="update-message" style="margin-top: 15px;display: flex;flex-direction: row;align-items: center;flex-wrap: wrap;gap: 10px;">
					<?php if ( 'valid' === $this->get_license_status() ) : ?>
						...
						<span><?php esc_html_e( 'License is valid.', 'utm-manager', 'utm-manager' ); ?></span>
					<?php elseif ( 'expired' === $this->get_license_status() ) : ?>
						<span class="dashicons dashicons-warning" style="color: #dc3232;"></span>
						<span><?php esc_html_e( 'License is expired.', 'utm-manager', 'utm-manager' ); ?></span>
					<?php elseif ( '' === $this->get_license_status() ) : ?>
						<span class="dashicons dashicons-warning" style="color: #dc3232;"></span>
						<span><?php esc_html_e( 'Please activate your license.', 'utm-manager', 'utm-manager' ); ?></span>
					<?php else : ?>
						<span class="dashicons dashicons-warning" style="color: #dc3232;"></span>
						<?php /* translators: %s: license status */ ?>
						<span><?php printf( esc_html__( 'License is %s.', 'utm-manager', 'utm-manager' ), esc_html( $this->get_license_status() ) ); ?></span>
					<?php endif; ?>
					<?php
					printf(
						...
						esc_attr__( 'Enter your license key', 'utm-manager', 'utm-manager' ),
					...
					printf(
						...
						esc_html__( 'Activate License', 'utm-manager', 'utm-manager' )
					);
					if ( 'valid' === $this->get_license_status() ) {
						printf(
							...
							esc_html__( 'Deactivate License', 'utm-manager', 'utm-manager' )
						);
						...
						printf(
							...
							esc_html__( 'Check License', 'utm-manager', 'utm-manager' )
						);
	...
	public function license_ajax_handler() {
		...
		if ( isset( $_POST['nonce'] ) && ! wp_verify_nonce( wp_unslash( $_POST['nonce'] ), $this->get_basename() . '_license_action' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			wp_send_json_error( __( 'Invalid nonce', 'utm-manager', 'utm-manager' ) );
		}

		if ( ! isset( $_POST['operation'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid operation', 'utm-manager', 'utm-manager' ) ) );
		}

		$operation = sanitize_text_field( wp_unslash( $_POST['operation'] ) );
		$license   = isset( $_POST['key'] ) ? sanitize_text_field( wp_unslash( $_POST['key'] ) ) : '';
		if ( empty( $license ) ) {
			wp_send_json_error( array( 'message' => __( 'License key is required', 'utm-manager', 'utm-manager' ) ) );
		}

		switch ( $operation ) {
			case 'activate_license':
				if ( $this->is_license_valid() && $license === $this->get_license_key() ) {
					wp_send_json_error( array( 'message' => __( 'License key is already activated', 'utm-manager', 'utm-manager' ) ) );
				}
					...
					wp_send_json_error(
						array(
							...
							/* translators: %s: error message */
								__( 'License activation failed. %s', 'utm-manager', 'utm-manager' ),
				...
				if ( isset( $api_data->license ) && 'valid' === $api_data->license ) {
					...
					wp_send_json_success(
						array(
							'message' => __( 'License activated successfully.', 'utm-manager', 'utm-manager' ),
					...
					wp_send_json_error(
						array(
							'message' => __( 'License activation failed.', 'utm-manager', 'utm-manager' ),
							'data'    => $api_data,
						)
					);
				}
			...
			case 'deactivate_license':
				...
				if ( is_wp_error( $api_data ) || ! $api_data->success ) {
					wp_send_json_error(
						array(
							'message' => sprintf(
							/* translators: %s: error message */
								__( 'License deactivation failed. %s', 'utm-manager', 'utm-manager' ),
								esc_html( is_wp_error( $api_data ) ? $api_data->get_error_message() : $api_data->error_message )
							),
							'data'    => $api_data,
						)
				...
				if ( isset( $api_data->license ) && 'deactivated' === $api_data->license ) {
					...
					wp_send_json_success(
						array(
							'message' => __( 'License deactivated successfully', 'utm-manager', 'utm-manager' ),
							'code'    => $api_data->license,
							'reload'  => true,
						)
					);
					...
					wp_send_json_error(
						array(
							'message' => __( 'License deactivation failed', 'utm-manager', 'utm-manager' ),
							'code'    => $api_data->license,
						)
					);
				}
			...
			case 'check_license':
				...
				if ( is_wp_error( $api_data ) || ! $api_data->success ) {
					wp_send_json_error(
						array(
							'message' => sprintf(
							/* translators: %s: error message */
								__( 'License check was failed. %s', 'utm-manager', 'utm-manager' ),
								esc_html( is_wp_error( $api_data ) ? $api_data->get_error_message() : $api_data->error_message )
							),
							'data'    => $api_data,
						)
				...
				if ( isset( $api_data->license ) && 'valid' === $api_data->license ) {
					...
					delete_transient( $this->data['prefix'] . '_latest_version' );
					delete_site_transient( 'update_plugins' );
					$message = __( 'Your license key is valid.', 'utm-manager', 'utm-manager' );
					...
					if ( isset( $api_data->activations_left ) && $api_data->activations_left > 0 ) {
						...
						$message .= ' ' . sprintf( __( 'You have %s activations left.', 'utm-manager', 'utm-manager' ), number_format( $api_data->activations_left ) );
					}
					// if set expiration date.
					if ( isset( $api_data->expires ) && 'lifetime' !== $api_data->expires ) {
						/* translators: %s: expiration date */
						$message .= ' ' . sprintf( __( 'Your license key expires on %s.', 'utm-manager', 'utm-manager' ), date_i18n( get_option( 'date_format' ), strtotime( $api_data->expires ) ) );
					}
					wp_send_json_success(
						array(
							'message' => $message,
					...
					wp_send_json_error(
						array(
							'message' => __( 'License is invalid', 'utm-manager', 'utm-manager' ),
							'code'    => $api_data->license,
						)
					);
				}
			...
			default:
				wp_send_json_error(
					array(
						'message' => __( 'Invalid action', 'utm-manager', 'utm-manager' ),
						'code'    => 'invalid_action',
					)
				);
				break;
	...
	public function plugins_api_filter( $result, $action, $args ) {
		...
		if ( ! $this->is_license_valid() ) {
			$request->package               = '';
			$request->sections['changelog'] = sprintf( esc_html__( 'Please activate your license key to get the latest updates and changelog.', 'utm-manager', 'utm-manager' ), $this->data['name'] );
		}
	...
	protected function get_license_api_data( $license, $action, $args = array() ) {
		...
		if ( ! is_object( $api_data ) ) {
			return new \WP_Error( 'error', __( 'Something went wrong. Please try again.', 'utm-manager', 'utm-manager' ) );
		}
			...
			switch ( $api_data->error ) {
				case 'expired':
					...
					/* translators: %s: license key */
						__( 'Your license key expired on %s.', 'utm-manager', 'utm-manager' ),
						date_i18n( get_option( 'date_format' ), strtotime( $api_data->expires ) )
					);
					break;
				case 'revoked':
				case 'disabled':
					$message = __( 'Your license key has been disabled. Please contact support.', 'utm-manager', 'utm-manager' );
					break;
				case 'missing':
					$message = __( 'Invalid license.', 'utm-manager', 'utm-manager' );
					break;
				case 'invalid':
				case 'site_inactive':
					$message = __( 'Your license is not active for this URL.', 'utm-manager', 'utm-manager' );
					break;
				case 'item_name_mismatch':
				case 'invalid_item_id':
					/* translators: %s: plugin name */
					$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'utm-manager', 'utm-manager' ), esc_html( $this->data['name'] ) );
					break;
				case 'no_activations_left':
					$message = __( 'Your license key has reached its activation limit.', 'utm-manager', 'utm-manager' );
					break;
				default:
					$message = __( 'An error occurred, please try again.', 'utm-manager', 'utm-manager' );
					break;
			}
