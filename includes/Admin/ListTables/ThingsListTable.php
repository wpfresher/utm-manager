...
class ThingsListTable extends \WP_List_Table {
	...
	public function prepare_items() {
		...
		$per_page              = $this->get_items_per_page( 'utmm_things_per_page', 20 );
		 ...
		 * @since 1.0.0
		 */
		$args = apply_filters( 'utm_manager_things_table_query_args', $args );
	...
	public function column_name( $item ) {
		$view_url   = add_query_arg( array( 'view' => $item->ID ), admin_url( 'admin.php?page=things-list-table' ) );
		$delete_url = add_query_arg(
			array(
				'ids'    => $item->ID,
				'action' => 'delete',
			),
			admin_url( 'admin.php?page=things-list-table' )
		);
